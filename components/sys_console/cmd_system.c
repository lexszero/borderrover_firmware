...
/** 'timers' commands dumps the list of timers */
static int timers_info(int argc, char **argv)
{
	esp_timer_dump(stdout);
	return 0;
}

static void register_timers()
{
	const esp_console_cmd_t cmd = {
		.command = "timers",
		.help = "Get information about active timers",
		.hint = NULL,
		.func = &timers_info,
	};
	ESP_ERROR_CHECK( esp_console_cmd_register(&cmd) );
